...
def inject_css():
    ...
    <style>
      ...
      h1, h2, h3 {{ color: {BRAND['ink']}; letter-spacing: -0.02em; }}
      .hero {{
        background: linear-gradient(120deg, {BRAND['primary']}14, {BRAND['secondary']}10);
        border: 1px solid {BRAND['primary']}22; border-radius: 18px;
        padding: 20px 26px; margin-bottom: 14px;
      }}
      .hero h1 {{ margin: 0; font-size: 1.7rem; }}
      .hero p  {{ margin: 4px 0 0; color: {BRAND['muted']}; font-size: .95rem; }}
      .exec-summary {{
        background: white; border-left: 5px solid {BRAND['primary']};
        border-radius: 10px; padding: 14px 18px; margin: 6px 0 18px;
        box-shadow: 0 1px 3px rgba(15,23,42,.06); font-size: .96rem; line-height: 1.55;
      }}
      .alert {{ border-radius: 10px; padding: 11px 15px; margin: 6px 0; font-size: .9rem; }}
      .alert-bad  {{ background: {BRAND['bad']}12;  border-left: 4px solid {BRAND['bad']};  }}
      .alert-warn {{ background: {BRAND['warn']}14; border-left: 4px solid {BRAND['warn']}; }}
      .alert-ok   {{ background: {BRAND['ok']}12;   border-left: 4px solid {BRAND['ok']};   }}
      div[data-testid="stMetric"] {{
        background: white; border: 1px solid #E2E8F0; border-radius: 14px;
        padding: 14px 16px; box-shadow: 0 1px 2px rgba(15,23,42,.05);
      }}
      div[data-testid="stMetricLabel"] {{ color: {BRAND['muted']}; font-weight: 600; }}
...
def style_fig(fig, h=340):
    fig.update_layout(
        ...
        colorway=SEQ, font=dict(size=13, color=BRAND["ink"]),
